...
export const updateCart = createAsyncThunk(
    ...
    async (formData: { id: string; data: { qty: number } }, thunkAPI) => {
        try {
            return await productService.updateCart(
                formData?.id,
                formData?.data
            );
        } catch (error: any) {
...
export const getMerchantsProducts = createAsyncThunk(
    ...
    async (data: { slug: string }, thunkAPI) => {
        try {
            return await productService.getMerchantsProducts(
                data.slug,
                data.slug
            );
        } catch (error: any) {
...
const productSlice = createSlice({
        ...
        syncLocalCartFromStorage: (state) => {
            ...
            if (typeof window !== 'undefined') {
                try {
                    ...
                    console.error(
                        'Error syncing cart from localStorage:',
                        error
                    );
                }
            }
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase('persist/REHYDRATE' as any, (state: any, action: any) => {
                ...
                if (typeof window !== 'undefined') {
                    try {
                        ...
                        console.error(
                            'Error syncing cart from localStorage on rehydrate:',
                            error
                        );
                    }
                }
            ...
            .addCase(
                getAllProductBaseOnCategories.fulfilled,
                (state, action) => {
                    state.isLoading = false;
            ...
            .addCase(
                getAllProductBaseOnCategories.rejected,
                (state, action) => {
                    state.isLoading = false;
                    state.error = true;
                    state.message = action.payload;
            ...
            .addCase(
                getAllProductBaseOnSubCategories.fulfilled,
                (state, action) => {
                    state.isLoading = false;
            ...
            .addCase(
                getAllProductBaseOnSubCategories.rejected,
                (state, action) => {
                    state.isLoading = false;
                    state.error = true;
                    state.message = action.payload;
            ...
            .addCase(
                getAllProductBaseOnSecondLevelSubCategories.pending,
                (state) => {
                    state.isLoading = true;
                }
            )
