import { copyTelegramConnectUrl, openTelegramConnectUrl } from "@/lib/openTelegramConnect";
...
export default function TelegramConnectModal({
  ...
  const handleOpenTelegram = () => {
    if (!connectUrl) return;
    const opened = openTelegramConnectUrl(connectUrl);
    if (!opened) {
      toast.message("Pop-up blocked", {
        description: "Use the link below or copy it to open Telegram in a new tab.",
      });
    }
  };
      ...
      >
        <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl dark:bg-gray-900">
          ...
          <p className="mt-2 text-sm text-gray-600 dark:text-gray-400">
            You are linked to Hawola Alerts on @{botUsername}. Order, shipping, chat, and other alerts you enable will
  ...
  return (
    ...
    >
      <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl dark:bg-gray-900">
        ...
        <p className="mt-2 text-sm text-gray-600 dark:text-gray-400">
          This page stays open. Open the link below in Telegram, tap Start, and we will confirm here automatically.
        </p>

        {loading || !connectUrl ? (
          <div className="mt-6 flex flex-col items-center gap-3 py-4">
            <div className="h-10 w-10 animate-spin rounded-full border-2 border-primary border-t-transparent" />
            <p className="text-sm text-gray-600 dark:text-gray-400">Preparing your connect link…</p>
          </div>
        ) : (
          <>
            {waiting ? (
              <p className="mt-3 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs font-medium text-amber-800 dark:border-amber-800 dark:bg-amber-900/20 dark:text-amber-200">
                Waiting for you to tap Start in Telegram… this popup will update when connected.
              </p>
            ) : null}

            <ol className="mt-4 list-decimal space-y-2 pl-4 text-sm text-gray-700 dark:text-gray-300">
              <li>
                Tap <strong>Open Telegram in new tab</strong> below (or use the link).
              </li>
              <li>
                In Telegram, tap <strong>Start</strong> on @{botUsername}.
              </li>
              <li>Return to this tab — we will confirm automatically.</li>
            </ol>
            ...
            <a
              ...
              className="mt-4 block break-all rounded-lg border border-gray-200 bg-gray-50 px-3 py-2 text-xs text-primary underline-offset-2 hover:underline dark:border-gray-700 dark:bg-gray-800"
            >
              {connectUrl}
            </a>
