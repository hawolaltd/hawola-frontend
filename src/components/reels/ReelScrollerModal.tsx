...
interface ReelScrollerModalProps {
  ...
  open: boolean;
  onClose: () => void;
  /** Shown when the product has no playable reels. */
  emptyMessage?: string;
}

export function ReelScrollerModal({
  reels,
  initialIndex,
  open,
  onClose,
  emptyMessage = "Reel not available for this product.",
}: ReelScrollerModalProps) {
    ...
    >
      ...
      {playable.length === 0 ? (
        <div className="flex min-h-0 flex-1 flex-col items-center justify-center gap-4 px-6 text-center">
          <p className="max-w-sm text-base font-medium text-white/90">{emptyMessage}</p>
          <button
            type="button"
            onClick={onClose}
            className="rounded-lg border border-white/20 bg-white/10 px-5 py-2.5 text-sm font-medium text-white hover:bg-white/15"
          >
            Close
          </button>
        </div>
          ...
          {playable.length > 1 ? (
            <div className="absolute right-3 top-1/2 z-20 hidden -translate-y-1/2 flex-col gap-2 sm:flex">
              <button
                ...
                disabled={!hasPrev}
                className="rounded-full border border-white/15 bg-black/50 p-2 text-white/90 backdrop-blur-sm transition hover:bg-black/70 disabled:pointer-events-none disabled:opacity-30"
