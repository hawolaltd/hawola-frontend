...
const ProductSlider = ({subcategory}: {subcategory: any}) => {
                    ...
                    <div key={category.id} className="px-2">
                        <div className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow duration-300 hover:border hover:border-orange">
                            <div className="p-4 flex gap-4">
                                <div>
                                    <img className="w-full h-20 object-cover" src={category?.image?.image_url} alt="category-1"/>
                                </div>

                                <div>
                                    {/*<h3 className="text-xs font-normal text-smallHeaderText mb-1">{category.brand}</h3>*/}
                                    <h4 className="text-sm font-bold text-primary mb-1">{category?.name}</h4>
                                    {/*<p className="flex items-center text-xs text-smallHeaderText mb-3"> {Array.from((category?.rating ?? 0)).map((star, key) => (*/}
                                    ...
                                    {/*            fill="#FFB067"/>*/}
                                    {/*    </svg>))}({category.rating})</p>*/}

                                    {/*<div className="flex items-center justify-between">*/}
                                    {/*    <div>*/}
                                    {/*        <span className="text-lg font-bold text-primary">{formatCurrency(category?.price)}</span>*/}
                                    {/*        <span className="text-sm text-smallHeaderText line-through ml-2">{category.discount_price}</span>*/}
                                    {/*    </div>*/}
                                    {/*</div>*/}
                                </div>
                            </div>
